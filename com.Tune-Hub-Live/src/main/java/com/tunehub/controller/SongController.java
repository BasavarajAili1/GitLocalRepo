...
//    	songService.deleteSong(songId);								// Delete the song from the database
    	List<Song> songslist = songService.fetchAllSongs();				// Fetch all songs from the database
        model.addAttribute("songslist", songslist);						// Add songs to the model for rendering in the view
    	return "songs/deletesongs";
    }
    
    @PostMapping("/deletesongs")
	public String deleteSongs(@RequestParam("songId") int songId) {
    	songService.deleteSong(songId);					// Delete the playlist from the database
	    return "redirect:/songs/deletesongs";
	}
}
